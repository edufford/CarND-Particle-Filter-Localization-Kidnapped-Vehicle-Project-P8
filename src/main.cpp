 ...
 * uWebSocket messages.  After GPS and sensed landmark measurements are
 * received, process it using a particle filter (predict, update, resample)
 * and send resulting best particle (highest weight) x, y, and theta position
 * estimates and associated sensed landmarks back to the simulator for
 * visualization.
 */
int main() {
  uWS::Hub h;
  ...
  // GPS measurement and movement uncertainty [x [m], y [m], theta [rad]]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (!pf.Initialized()) {
            // Sense noisy GPS position data from the simulator
            double sense_x = std::stod(j[1]["sense_x"].get<std::string>());
            double sense_y = std::stod(j[1]["sense_y"].get<std::string>());
            double sense_theta = std::stod(j[1]["sense_theta"]
                                           .get<std::string>());
            
            // Initialize particle filter with GPS position
            ...
            // Move particles with control data and add some uncertainty
            pf.Prediction(delta_t, sigma_pos,
          ...
          // Update the particle weights and resample
          ...
          std::cout << "best particle x, y, theta: "
                    << best_particle.x << ", " << best_particle.y << ", "
                    << best_particle.theta << std::endl;
